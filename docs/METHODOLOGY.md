...
| **Mean Time to First Detection** | N/A ($0.00\text{s}$) | **0.60s** | Simulated event-rule alert timestamp |
| **Mean Time to Actionable Detection** | N/A ($0.00\text{s}$) | **0.60s** | First HIGH/CRITICAL containment-triggering alert |
...
| **Avg Successful Lateral Steps / Adversarial Run** | **9.0** | **0.0** | Segmentation and isolation attenuation |
| **Avg Sensitive/Backup Assets Reached / Adversarial Run** | **4.0** | **0.0** | Tier-0 exposure eliminated |
| **Mean Post-Control Risk Score**| **52.968 pts** | **16.0 pts** | $-37.0$ points risk reduction |
| **Avg Files Affected / Adversarial Run** | **5.0 files** | **0.0 files** | Calculated only from actual attack attempts |
| **File Protection Rate (%)** | **0.0%** | **100.0%** | Denominator is the recorded target count |
...
2. **Defense-in-Depth Attribution:** Even when a compromised vendor identity already holds a valid authenticated session, the modeled controls reduce successful lateral movement from **9.0 to 0.0 steps per adversarial run** and sensitive/backup assets reached from **4.0 to 0.0**. Detection then triggers the modeled containment workflow before any harmless ransomware marker is written in the protected configuration.
3. **Automated Containment as a Control Objective:** The model implements automated in-memory session isolation with a measured **0.60s simulated** containment duration. This demonstrates the role of automated containment in this synthetic control design; it is not a universal real-world operational requirement or SOC timing claim.
...
4. **Benign-Session Scope:** Benign trials use a simulated valid vendor password and MFA token, then pass through the same `Authenticator` and `AccessControl` layers as other sessions. They model one permitted vendor-zone read and are used to measure legitimate-access success and false-positive behavior; they do not model a broad range of normal operational workflows.
